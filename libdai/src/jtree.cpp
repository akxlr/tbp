...
//        cout << "jtree.cpp creating factor with vars " << IR(beta) << endl;
...
std::vector<Eigen::VectorXd> JTree::full_prod_all_beliefs() {

    cerr << "Finding marginals based on full_prod accumulated at root" << endl;

    // full_prod contains the full product of all clique potentials accumulated at the root, we just need to
    // marginalise for all vars
    ...
    size_t n_vars = full_prod.factors.size();
    std::vector<Eigen::VectorXd> colsums(n_vars);
    for (size_t i=0; i<n_vars; ++i) {
        colsums[i] = full_prod.factors[i].colwise().sum();
        // Multiply column sum onto weights (we store colsums so we can later divide out each variable in turn)
        full_prod.weights.array() *= colsums[i].array();
    }

    // Compute marginals by simplifying each variable to use only one term
    std::vector<Eigen::VectorXd> res(n_vars);
    for (size_t i=0; i<n_vars; ++i) {
        // Matrix-vector full_product
        res[i] = full_prod.factors[i] * (full_prod.weights.array() / colsums[i].array()).matrix();
    }
...
//            message( j, _e ) = msg.marginal( IR(e), false );
            ...
            DAI_THROWE(RUNTIME_ERROR,"Mult-all not supported for maxprod");
...
//    cerr << "Finished first pass, computing marginals immediately from message product at root" << endl;
    ...
    // product is something like: OR(root) * message(root, i) for i in incoming messages
    full_prod = OR(0); // 0 is the root note, OR(0) is the root node's clique potential
    bforeach( const Neighbor &k, nbOR(0) )
        full_prod *= message( 0, k.iter );
